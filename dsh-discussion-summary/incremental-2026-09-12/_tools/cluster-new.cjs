#!/usr/bin/env node
/**
 * Cluster new discussions (#5886+) into distinct problem families.
 *
...
const FAMILIES = [
  ...
  },
  {
    ...
    name: '升级后历史会话无法加载',
    kw: [/历史.*无法加载|无法加载.*历史|历史加载失败|history.*(fail|unavailable|load)/i, /session.*unreadable|unreadable.*session/i, /会话.*永久.*(打不开|不可用|无法)/i, /打不开.*会话|会话.*打不开/i, /Failed to load history/i, /cannot safely transform/i, /unknown to this harness/i],
  },
  {
    id: 'fork-inbox',
    name: 'Fork 会话继承父会话队列',
