...
| 顺序 | 文件（点击直达） | 关注点 | 对应需求 |
|---|---|---|---|
| 1 | [packages/interaction/user-approval/src/types.ts](../deepseek-harness/packages/interaction/user-approval/src/types.ts) | `ApprovalRequestId`、`ApprovalOutcome`（第 17~32 行） | R1/R2 |
...
| 6 | [packages/goal/tool-goal/src/index.ts](../deepseek-harness/packages/goal/tool-goal/src/index.ts) | create_goal / get_goal / update_goal 工具 | R3 |

### 2.2 关键实现片段

**片段 A：闭集且失败关闭的审批结果**（[user-approval/src/types.ts 第 29~32 行](../deepseek-harness/packages/interaction/user-approval/src/types.ts)）

```ts
type ApprovalOutcome = 'allowed-once' | 'rejected' | 'cancelled' | 'unavailable'
```

翻译：审批结果是一个**闭集**——`allowed-once` 只授予被问的这一次动作；`rejected` 显式拒绝；`cancelled` 请求被撤回；`unavailable` 回答者缺失/异常。官方注释强调：*"A missing, non-owning, throwing, or non-conforming answerer becomes `unavailable` rather than opening the gate"*（第 30 行）——**闸门故障 = 关闭**（P1）。

**片段 B：目标更新的 CAS 身份**（[goal/src/types.ts 第 20~25 行](../deepseek-harness/packages/goal/goal/src/types.ts)）
...
**片段 C：计划的纯折叠恢复**（[plan-mode/src/index.ts 第 46~48 行](../deepseek-harness/packages/plan/plan-mode/src/index.ts)）
...
**片段 D：目标的激活状态**（[goal/src/types.ts 第 71~72 行](../deepseek-harness/packages/goal/goal/src/types.ts)）

```ts
export type GoalActivation = 'armed' | 'disarmed'
```

翻译：`GoalActivation` 是**进程本地**的状态，不持久化。`armed` 表示当前进程可以自动继续目标；`disarmed` 表示目标暂停（如会话恢复后）。这区分了**持久阶段**（GoalPhase）和**运行时激活**（GoalActivation）。

### 2.3 符号 hover 指引

在 VS Code 打开 [packages/interaction/user-approval/src/types.ts](../deepseek-harness/packages/interaction/user-approval/src/types.ts) hover `ApprovalOutcome`；打开 [packages/goal/goal/src/types.ts](../deepseek-harness/packages/goal/goal/src/types.ts) hover `GoalRef` 与 `GoalPhase`。
...
打开 [packages/plan/plan-mode/src/index.ts](../deepseek-harness/packages/plan/plan-mode/src/index.ts)，找到 `plan/mode` 事件定义；再对照 [plan.md 第 11 行](../deepseek-harness/docs/subsystems/plan.md) 的官方描述——"the state in force is always a pure fold of the session log"。

### 任务 4（进阶）：查看目标的激活状态

打开 [packages/goal/goal/src/types.ts](../deepseek-harness/packages/goal/goal/src/types.ts)，找到 `GoalActivation` 类型（第 72 行），理解 `armed` 和 `disarmed` 的区别——这是**进程本地**状态，不持久化。
...
- **Q6：GoalActivation 和 GoalPhase 有什么区别？** GoalPhase 是持久化的目标阶段（active/paused/blocked/complete），GoalActivation 是进程本地的运行时状态（armed/disarmed），不持久化。
...
6. `GoalActivation` 的两个取值？它与 `GoalPhase` 的关系？
...
4. 纯折叠：`plan/mode` 从会话日志派生，无 live 镜像，resume/fork/compaction 自动恢复。
5. C。沙箱模式属于 `sandbox` 组（第 05 篇）。
6. armed / disarmed。GoalPhase 是持久化的目标阶段（active/paused/blocked/complete），GoalActivation 是进程本地的运行时状态（armed/disarmed），不持久化。
...
|------|-------------|-------------|----------|
| **核心架构** | approval/goal/plan/guard 四层治理 | 保持不变 | 无变化 |
| **ApprovalOutcome** | 四值闭集 | 保持不变 | 无变化 |
| **GoalRef CAS** | id + revision | 保持不变 | 无变化 |
| **GoalPhase** | active/paused/blocked/complete | 保持不变 | 无变化 |
| **GoalActivation** | armed/disarmed | 保持不变 | 无变化 |
| **plan/mode** | 纯折叠恢复 | 保持不变 | 无变化 |
| **guard 插件** | repeat-tool-reminder + timeout-policy | 保持不变 | 无变化 |

### 6.2 核心架构稳定性

v0.1.5-rc.2 相对于 v0.1.0-rc.5 的变更**集中在 UI 层和反馈系统**，核心的人机协作与治理机制（approval、goal、plan、guard）保持**完全稳定**：

```mermaid
graph LR
    subgraph v0.1.0-rc.5
        A1[approval] --> B1[闭集结果]
        A2[goal] --> B2[CAS revision]
        A3[plan] --> B3[纯折叠]
        A4[guard] --> B4[提醒+超时]
    end
...
### 6.3 v0.1.5-rc.2 的实际变更

根据变更日志，v0.1.5-rc.2 的核心变更是：

| 变更类型 | 详情 | 与治理机制的关系 |
|----------|------|------------------|
| **对称消息反馈** | Like/Dislike 统一走对话框 | 无直接关系（UI 交互） |
| **共享文件图标** | 50+ 种文件类型图标重构 | 无直接关系（UI 展示） |
| **交付物卡片精化** | 卡片尺寸缩小 ~17%，间距优化 | 无直接关系（UI 布局） |
| **包版本号批量升级** | ~300 个 package.json 版本号 | 无直接关系（版本管理） |

### 6.4 架构不变的意义

**为什么核心治理机制保持不变？**

1. **P1-P5 原则依然适用**：闸门失败关闭、状态即日志、单写路径、职责分离、卫生显式——这些设计哲学没有过时；
2. **API 兼容性**：tool-goal（create_goal/get_goal/update_goal）、exit_plan_mode 工具接口保持稳定；
3. **事件格式稳定**：approval/asked、approval/decided、plan/mode 事件格式未变；
4. **CAS 语义稳定**：GoalRef 的 revision 递增机制未变。

### 6.5 升级建议

**从 v0.1.0-rc.5 升级到 v0.1.5-rc.2**：

| 组件 | 升级影响 | 建议 |
|------|----------|------|
| **自定义 approval 回答者** | ✅ 无影响 | 无需改动 |
| **自定义 goal 扩展** | ✅ 无影响 | 无需改动 |
| **自定义 plan 工具** | ✅ 无影响 | 无需改动 |
| **自定义 guard 插件** | ✅ 无影响 | 无需改动 |
| **反馈系统扩展** | ⚠️ 需适配 | toggle→retract，openDialog 新增 rating 参数 |

**结论**：如果你只使用 approval/goal/plan/guard 的核心 API，升级到 v0.1.5-rc.2 是**无痛的**。只有涉及反馈系统扩展才需要适配。
...
**数据来源**：
- 源码：`E:\test\rewrite-agently\deepseek-harness\packages\interaction\`、`packages\goal\`、`packages\plan\`、`packages\guard\`
- 变更日志：`E:\test\rewrite-agently\dsh-docs-deliverables\source-analysis\v0.1.5-rc.1\changelog-alpha1-rc1.md`、`v0.1.5-rc.2\changelog-rc1-rc2.md`
- 参考文档：`E:\test\rewrite-agently\dsh-docs-deliverables\source-analysis\v0.1.0-rc.5\08-human-agent-governance.md`
