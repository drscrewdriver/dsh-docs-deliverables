...
- **Q4：崩溃恢复时 `interrupted` 是谁发的？** 后端加载日志时发现孤儿 `turn/start`，合成 `turn/end { reason: { kind: 'interrupted' } }` 关闭——这是唯一一个循环自己不会发出的 TurnEndReason（[persistence.md 第 15 行](../deepseek-harness/docs/subsystems/persistence.md)）。
...
| 维度 | v0.1.0-rc.5 | v0.1.5-rc.2 | 变更类型 |
|------|-------------|-------------|----------|
| **Session 格式版本** | V0（`SESSION_FORMAT_VERSION = 0`） | V3（经 V0→V1→V2→V3 迁移） | 🔴 重大 |
| **Surface 折叠** | 基础 append/replace | 表面节点折叠 + 替换代数 | 🔴 重大 |
| **压缩事件类型** | compaction/start/summary/end | 新增 `compaction/prune` | 🟡 新增 |
| **压缩配置** | 阈值/保留比 | 按模型/提供者策略化配置 | 🟡 增强 |
| **工具结果修剪** | 无 | 可选 `compaction-tool-result-pruner` | 🟡 新增 |
| **投影缓存版本** | v6 | v7（兼容 v3-v6） | 🟢 迭代 |

### 6.2 Session V3 Surface Node（关键变更）

v0.1.5-rc.2 引入了 **Session V3** 格式，核心变更是 **Surface Node 折叠代数**：

```mermaid
flowchart TD
    A["Session Event Log<br/>(append-only)"] --> B["Surface Fold<br/>(运行时投影)"]
    B --> C["Surface Nodes<br/>(模型可见面)"]
    C --> D["deriveMessages<br/>(LLM 请求构建)"]
    
    B --> E["append 操作<br/>tail 追加"]
    B --> F["replace 操作<br/>范围替换"]
    
    E --> G["system/message<br/>user/message<br/>assistant/message<br/>tool/result"]
    F --> H["sourceEventSeqs<br/>被遮蔽节点引用"]
```
...
**折叠规则**（`packages/core/session/src/surface.ts`）：

| 操作 | 语义 | 效果 |
|------|------|------|
| `surfaceOp: 'append'` | 追加到表面尾部 | 新增一个表面节点 |
| `surfaceOp: { op: 'replace', startSeq, endSeq }` | 替换指定范围 | 移除旧节点，插入新节点 |
| `sourceEventSeqs` | 引用被遮蔽的源事件 | 保持血缘可追踪 |

### 6.3 compaction/prune 事件（v0.1.5-rc.2 新增）

新增 `compaction/prune` 事件类型，用于**模型无关的工具结果修剪**：
...
**与 compaction/summary 的区别**：

| 事件 | 触发方式 | 摘要生成 | 使用场景 |
|------|----------|----------|----------|
| `compaction/summary` | LLM 调用 | 有（通过 `ctx.llm.stream()`） | 语义压缩 |
| `compaction/prune` | 模型无关 | 无（直接裁剪） | 工具结果修剪 |

### 6.4 压缩策略化配置

v0.1.5-rc.2 支持**按模型/提供者配置不同的压缩策略**：

```typescript
// packages/compaction/compaction-basic/src/types.ts
interface BasicCompactionConfig {
  thresholdRatio: number    // 全局阈值比
  retainRatio: number       // 全局保留比
  modelPolicies: Array<{
    provider: string
    model: string
...
新增可选的 `compaction-tool-result-pruner` 包，提供**确定性的 head/middle/tail 修剪**：

```typescript
// packages/compaction/compaction-tool-result-pruner/src/index.ts
export class ToolResultPruner {
  pruneSession(session: Session): void
}
```
...
- **Head**：保留前 N 个字符
- **Tail**：保留后 N 个字符  
- **Middle**：替换为占位符 `[...pruned...]`

### 6.6 迁移路径

从 v0.1.0-rc.5 升级到 v0.1.5-rc.2 需要经历格式迁移：

```
Session V0 → V1 → V2 → V3
   │        │    │    │
   │        │    │    └─ Surface Node 折叠代数
   │        │    └─ Assistant Stream 基数变更
   │        └─ Identity 迁移
   └─ 原始格式
```

迁移包位于 `packages/session/session-format-v0-to-v1/` 和 `session-format-v1-to-v2/`，提供向后兼容的解码器。
...
- 🟡 [.agents/notes/implemented/architecture/2026-09-02-projcache-cross-version-read-compat.md](../deepseek-harness/.agents/notes/implemented/architecture/2026-09-02-projcache-cross-version-read-compat.md) —— 投影缓存跨版本兼容
...
1. 源码分析：`deepseek-harness/packages/session/`、`deepseek-harness/packages/session-query/`、`deepseek-harness/packages/compaction/`
2. 变更日志：`dsh-docs-deliverables/source-analysis/v0.1.5-rc.1/changelog-alpha1-rc1.md`、`dsh-docs-deliverables/source-analysis/v0.1.5-rc.2/changelog-rc1-rc2.md`
3. 参考文档：`dsh-docs-deliverables/source-analysis/v0.1.0-rc.5/06-memory.md`
4. 官方文档：`deepseek-harness/docs/subsystems/persistence.md`、`session-query.md`、`compaction.md`