...
- [5 FAQ 与自测](#5-faq-与自测❓)
- [6 延伸阅读](#6-延伸阅读📚)
- [7 版本演进](#7-版本演进v010-rc5--v015-rc2)
...
一个 Agent 要真正干活，必须有三个器官：**记忆**（记住聊了什么、做了什么）、**手脚**（真的去执行命令、读写文件）、**大脑**（决定每一步做什么、什么时候结束）。`packages/core/` 就是这三个器官的所在地——它是**每个组合都必须启动的主干**（官方称 "the packages every composition boots"）。
...
| 顺序 | 文件（点击直达） | 关注点 | 对应需求 |
|---|---|---|---|
| 1 | [packages/core/session/src/types.ts](../deepseek-harness/packages/core/session/src/types.ts) | `SessionEventMap`：事件词汇表（约第 269 行起） | R1/R3 |
...
| 5 | [packages/core/agent/src/types.ts](../deepseek-harness/packages/core/agent/src/types.ts) | `Agent` 接口、`InboxState`、`TurnBoundaryProjection` | R3/R4 |
...
翻译：这是所有 UI/hook/编排插件编程面对的**唯一表面**（R4）。`Agent` 接口暴露会话身份；`InboxState` 定义待处理消息的状态；`TurnBoundaryProjection` 提供 turn/step 边界的投影视图。注意它暴露 `session` 而不暴露"消息数组"——一切读操作都走日志派生（R1）。
...
<table style="border-collapse:collapse;width:100%;font-size:13px">
  ...
  <tr>
    ...
    <td style="border:1px solid #d0d7de;padding:5px 8px;font-family:monospace">{"type":"assistant/message","seq":9,…"stream":[{"type":"block-start","index":0,"blockType":"reasoning"}]}</td>
...
## 7 版本演进（v0.1.0-rc.5 → v0.1.5-rc.2）

### 7.1 版本定位

```
dsh-v0.1.0-rc.5  ← 旧版（早期 RC）
    │
    │  ← 约 140 个 commit（Alpha.1 → RC.1）
    │
dsh-v0.1.5-rc.2  ← 当前版（0.1.5 第二个 RC）
```
...
| 包名 | 版本变化 | 主要变化 |
|------|----------|----------|
| `@deepseek-ai/dsh-agent` | 0.1.0-rc.5 → 0.1.5-rc.2 | 新增 `Agent` 接口、`InboxState`、`InboxWireState`、`TurnBoundaryProjection` 类型；Typert 协议声明合并 |
| `@deepseek-ai/dsh-session` | 0.1.0-rc.5 → 0.1.5-rc.2 | 事件词汇表重构：新增 `assistant/attempt` 事件；`assistant/chunk` 被移除；`assistant/message` 增加 `stream` 和 `interrupted` 字段；`tool/call` 的 `callId` 类型从 `CallId` 改为 `ToolCallId` |
| `@deepseek-ai/dsh-tools` | 0.1.0-rc.5 → 0.1.5-rc.2 | 新增 PTC（Plan-then-Code）相关事件类型：`tool/ptc-dispatch-start` 和 `tool/ptc-dispatch` |
| `@deepseek-ai/dsh-agent-loop` | 0.1.0-rc.5 → 0.1.5-rc.2 | 版本号升级，无功能变化 |
| `@deepseek-ai/dsh-system-prompt` | 0.1.0-rc.5 → 0.1.5-rc.2 | 版本号升级，无功能变化 |
| `@deepseek-ai/dsh-scope` | 0.1.0-rc.5 → 0.1.5-rc.2 | 版本号升级，无功能变化 |
| `@deepseek-ai/dsh-agent-default-model` | 0.1.0-rc.5 → 0.1.5-rc.2 | 版本号升级，无功能变化 |
| `@deepseek-ai/dsh-agent-tool-presentation` | 0.1.0-rc.5 → 0.1.5-rc.2 | 版本号升级，无功能变化 |

### 7.3 关键变化详解

#### 7.3.1 Agent 接口增强

**v0.1.0-rc.5**：`Agent` 接口只包含 `id: SessionId`。

**v0.1.5-rc.2**：新增多个投影类型，支持更丰富的状态查询：
...
**影响**：UI 和钩子插件可以更精确地查询 agent 状态，无需遍历整个会话日志。

#### 7.3.2 事件词汇表重构

**v0.1.0-rc.5**：`assistant/chunk` 事件记录原始流块。

**v0.1.5-rc.2**：`assistant/chunk` 被移除，流块信息整合到 `assistant/message` 事件中：

```ts
// v0.1.5-rc.2 的 assistant/message 事件
'assistant/message': {
  turn: number
  step: number
  message: AssistantMessage
  stream: AssistantStreamRecord[]  // 新增：完整流记录
  usage?: TokenUsage
  interrupted?: true               // 新增：中断标记
}
```

**影响**：简化了事件词汇表，减少了事件数量；流记录与消息绑定，便于回放和调试。

#### 7.3.3 PTC 事件支持

**v0.1.0-rc.5**：无 PTC 支持。

**v0.1.5-rc.2**：新增 PTC（Plan-then-Code）相关事件：
...
**影响**：支持嵌套工具调用（如 `run_code` 内部调用其他工具），提供更好的可观测性。

### 7.4 版本演进总结

v0.1.0-rc.5 到 v0.1.5-rc.2 的变化主要体现在：

1. **接口增强**：Agent 接口新增投影类型，支持更丰富的状态查询
2. **事件简化**：移除 `assistant/chunk`，整合到 `assistant/message`，简化事件词汇表
3. **功能扩展**：新增 PTC 事件支持，为嵌套工具调用提供基础设施
4. **类型安全**：`callId` 类型从 `CallId` 改为 `ToolCallId`，增强类型安全性

这些变化体现了 Core 产品主干在保持架构稳定的前提下，逐步增强功能和类型安全性的演进方向。

---

**下一篇预告**：【第 03 篇】`packages/shell/`——能力缝：给 Agent 一双手（v0.1.5-rc.2 版本）。